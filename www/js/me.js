			...
			areaChoose = AreaChoose({
				...
				submit: function() {
					...
					$.ajax({
						...
						success: function(response) { 
							if (response.status == "success") {
								$("#address [fieldName='country']").text(area.country);
								$("#address [fieldName='province']").text(area.province);
								$("#address [fieldName='city']").text(area.city);
								
								areaChoose.setOriginArea({
									country: area.country,
									province: area.province,
									city: area.city
								}).scrollToOriginArea();
